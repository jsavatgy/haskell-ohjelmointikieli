...
mercator (Spheric3D lambda delta) = Point l d
  where
...
gridGreatCircles = concat [[[
  xpt l1 d2, xpt l2 d2, xpt l2 d1, xpt l1 d1, xpt l1 d2]
  | (d1,d2) <- zip vb3 (tail vb3)]
    | (l1,l2) <- zip vb2 (tail vb2)]
  where
    xpt l d = mercator (Spheric3D (DEG l) (DEG d))
    vb3 = visible3 delta
    vb2 = visible2 lambda
    delta = [-90,-75..90]
    lambda = [-90,-75..90]
...
layers1 = [
  ...
  Layer "4" Black [grid1] "[line width=0.8pt]",
  Empty
  ]

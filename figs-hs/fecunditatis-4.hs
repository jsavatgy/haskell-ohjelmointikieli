...
fc3 = concat [new io1 io2 p1 p2 
  ...
  where
    ...
    new In  In  p1 p2 = [p1]
    new In  Out p1 p2 = [p1,fromJust (intersection s1 s2 p1 p2)]
    new Out Out p1 p2 = []
    new Out In  p1 p2 = [fromJust (intersection s1 s2 p1 p2)]
...
ptLegends = 
  [Text pt ("$\\mathbf p_{" ++ show n ++ "}$") | (n,pt) <- zip [1..] pg2] ++
  [Text pt ("$\\bullet$") | (n,pt) <- zip [1..] pg]
  where
...
iPts1 = [ 
  NamedSymbPos "$\\mathbf i_{1}$" "$\\bullet$" SW n1,
  NamedSymbPos "$\\mathbf i_{2}$" "$\\bullet$" NW n2
  ]
  where
    [n1,n2] = map Node [pg !! 2, pg !! 3]
...
layers1 = [
  ...
  Layer "2" Gray20 (map Filled [fecunditatis3]) "[line width=0.8pt]",
